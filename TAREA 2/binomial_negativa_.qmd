...
# Función para comparar suma de geométricas y binomial negativa
comparar_binneg_geom <- function(r, p, n_sim) {
  # Simulación de la suma de geométricas
  suma_geometricas <- replicate(n_sim, {
    sum(rgeom(r, prob = p))
  })
  
  # Simulación directa de binomial negativa
  binomial_negativa <- rnbinom(n_sim, size = r, prob = p)
  
  # Crear data frame para comparación
  data_comparacion <- data.frame(
    valores = c(suma_geometricas, binomial_negativa),
    metodo = rep(c("Suma de r Geométricas", "Binomial Negativa"), 
                 each = n_sim)
  )
  ...
  ggplot(data_comparacion, aes(x = valores, fill = metodo)) +
    geom_histogram(alpha = 0.6, bins = 30, position = "identity") +
    facet_wrap(~metodo, ncol = 1) +
    labs(title = "Comparación: Suma de Geométricas vs Binomial Negativa",
         subtitle = paste("r =", r, ", p =", p, ", n =", format(n_sim, big.mark = ",")),
         x = "Número de fracasos",
         y = "Frecuencia") +
    theme_minimal() +
...
# Ejemplo de uso
comparar_binneg_geom(r = 4, p = 0.3, n_sim
 = 10000)
...
p_tiro <- 0.7
r_tiros <- 8
n_sim_baloncesto <- 1000

# a) Modelar con suma de geométricas
## Cada geométrica cuenta los tiros fallados antes de cada tiro exitoso
...
cat("EJERCICIO: Baloncesto\n")
cat("=====================\n")
cat("Número esperado de tiros fallados:", round(esperado_fallados, 2
), "±", round(desv_std_fallados, 2), "\n\n")

# c) Simulación de sesiones de entrenamiento
set.seed(123)
...
cat("Media simulada de tiros fallados en", n_sim_baloncesto, "ses
iones:", round(media_sim_baloncesto, 2), "\n")
cat("Diferencia con el valor teórico:", abs(esperado_fallados -
    media_sim_baloncesto), "\n")

# Gráfico de la distribución simulada
ggplot(data.frame(fallados = simulacion_baloncesto), aes(x = fallados)) +
  geom_histogram(aes(y = after_stat(density)), bins = 30, fill = "#3498DB", alpha = 0.7) +
  geom_vline(xintercept = esperado_fallados, color = "#E74C3C", 
             linetype = "dashed", linewidth = 1) +
  labs(title = "Distribución de Tiros Fallados en Sesiones de Entrenamiento",
       subtitle = paste("p =", p_tiro, ", r =", r_tiros, 
                       ". Línea roja: esperado =", round(esperado_fallados, 2)),
...
p_exito_lab <- 0.3
r_exitos_lab <- 4
k_fallidas_exactas <- 12
n_sim_lab <- 10000

# a) Probabilidad de exactamente 12 pruebas fallidas
prob_exactas <- dnbinom(k_fallidas_exactas, size = r_exitos_lab, prob = p_exito_lab)

# b) Probabilidad de más de 20 pruebas fallidas
prob_mas_20 <- 1 - pnbinom(20, size = r_exitos_lab, prob = p_exito_lab)

# c) Costo esperado en pruebas fallidas
costo_por_prueba <- 10000
esperado_fallidas_lab <- r_exitos_lab * (1 - p_exito_lab
) / p_exito_lab
costo_esperado <- esperado_fallidas_lab * costo_por_prueba
cat("EJERCICIO: Biotecnología\n")
cat("=========================\n")
cat("a) P(X = 12) =", round(prob_exactas, 6),
    "\n")
cat("b) P(X > 20) =", round(prob_mas_20,
    6), "\n")
cat("c) Costo esperado en pruebas fallidas: $", round(costo_esperado, 2), "\n")

# Simulación 
set.seed(456)
simulacion_lab <- replicate(n_sim_lab, {
  sum(rgeom(r_exitos_lab, prob = p_exito_lab))
})
media_sim_lab <- mean(simulacion_lab)
cat("Media simulada de pruebas fallidas en", n_sim_lab, "simulaciones
:", round(media_sim_lab, 2), "\n")
cat("Diferencia con el valor teórico:", abs(esperado_fallidas_lab - 
    media_sim_lab), "\n")
# Gráfico de la distribución simulada
ggplot(data.frame(fallidas = simulacion_lab), aes(x = fallidas)) +
  geom_histogram(aes(y = after_stat(density)), bins = 30, fill = "#3498DB", alpha = 0.7) +
  geom_vline(xintercept = esperado_fallidas_lab, color = "#E74C3C", 
             linetype = "dashed", linewidth = 1) +
  labs(title = "Distribución de Pruebas Fallidas en el Laboratorio",
       subtitle = paste("p =", p_exito_lab, ", r =", r_exitos_lab, 
                       ". Línea roja: esperado =", round(esperado_fallidas_lab, 2)),
