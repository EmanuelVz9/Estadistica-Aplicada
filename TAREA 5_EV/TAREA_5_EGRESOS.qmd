...
egresos_cor <- egresos |>
filter(!is.na(CATEGORIA_PRINCIPAL)) |>
group_by(ANIO, CVE_ENT, ABR_ENT, CATEGORIA_PRINCIPAL) |>
summarise(valor = sum(VALOR_millones, na.rm = TRUE), .groups = "drop")

totales <- egresos_cor |>
group_by(ANIO, CVE_ENT) |>
summarise(total = sum(valor), .groups = "drop")

egresos_cor <- left_join(egresos_cor, totales, by=c("ANIO","CVE_ENT")) |>
mutate(porcentaje = valor / total * 100)

# 2. Pivot a formato ancho

egresos_cor_wide <- egresos_cor |>
mutate(CATEGORIA_PRINCIPAL = make.names(CATEGORIA_PRINCIPAL)) |>
pivot_wider(
names_from = CATEGORIA_PRINCIPAL,
...
# 3. Seleccionar columnas numéricas con base R (NO usa where())

numericas <- sapply(egresos_cor_wide, is.numeric)
matriz_numerica <- egresos_cor_wide[, numericas]
...
round(3) |>
kable(
caption = "Matriz de correlación entre categorías",
digits = 3
) |>
kable_styling(
